...
import { Copy, CheckCircle2, Download, Sparkles, Smartphone, Save, Loader2 } from "lucide-react";
...
export function generateSemanticTheme(baseColor: string, isDark: boolean = false): SemanticTheme {
  try {
...
export const MobileMockup = ({ theme, isDark }: { theme: SemanticTheme, isDark: boolean }) => {
   return (
      <div 
...
export function ProjectThemeEditor({ project }: { project: any }) {
  const [baseColor, setBaseColor] = useState(project.base_color);
  const [copied, setCopied] = useState(false);
  const [saving, setSaving] = useState(false);
  const [saveSuccess, setSaveSuccess] = useState(false);

  // Sync with prop if it loads late
  useEffect(() => {
     if(project.base_color) setBaseColor(project.base_color);
  }, [project.base_color]);
  ...
  const outputJson = useMemo(() => ({ light: lightTheme, dark: darkTheme }), [lightTheme, darkTheme]);
  ...
  const handleSaveToProject = async () => {
      setSaving(true);
      const res = await saveTheme(project.id, outputJson);
      setSaving(false);
        ...
        >
           <div className="rounded-3xl border border-border/50 bg-card p-6 shadow-xl backdrop-blur-md sticky top-24">
              <h2 className="text-xl font-bold mb-6 flex items-center">
                 <Sparkles className="mr-2 h-5 w-5 text-blue-500" /> Adjust Color
              </h2>
              ...
              <div className="mt-8 pt-6 border-t border-border/50 space-y-3">
                 <Button onClick={copyToClipboard} variant="outline" className="w-full h-12 text-md font-medium rounded-xl">
                    {copied ? <CheckCircle2 className="mr-2 h-5 w-5 text-emerald-500" /> : <Copy className="mr-2 h-5 w-5" />}
                 ...
                 <Button 
                    ...
                    disabled={saving}
                    className="w-full h-12 text-md font-medium rounded-xl shadow-lg transition-transform active:scale-95" 
                    style={{ backgroundColor: baseColor, color: chroma.contrast(baseColor, 'white') > 4.5 ? 'white' : 'black' }}
                 >
                    {saving ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : 
                     saveSuccess ? <CheckCircle2 className="mr-2 h-5 w-5" /> : 
                     <Save className="mr-2 h-5 w-5" />}
                    {saveSuccess ? 'Saved to Project' : 'Save to Project'}
                 </Button>
              </div>
           </div>
        </motion.div>
